...
test('개인 재판 줄은 연속 무죄 일수를 보여준다', () => {
    /*
     * 남은 한도는 바로 위 탕이 말풍선이 이미 말한다 — 같은 문장을 한 화면에 두 번 띄우지 않는다(#450).
     * 이 줄은 「오늘」이 아니라 「누적」을 맡는다.
     */
    assert.deepEqual(getHomePersonalTrialRow(8), { text: '연속 무죄 8일째', badge: null });

    /* 연속 기록이 끊겼으면 0일이라고 자랑하지 않는다. */
    assert.match(getHomePersonalTrialRow(0).text, /아직 연속 무죄 기록이 없어요/);
    assert.match(getHomePersonalTrialRow(null).text, /아직 연속 무죄 기록이 없어요/);
    assert.match(getHomePersonalTrialRow(undefined).text, /아직 연속 무죄 기록이 없어요/);
});
