...
**화면 작업 전에 `http://localhost:5173/dev/ui` 를 먼저 연다.**
공통 컴포넌트·컬러 토큰·타이포 스케일이 전부 모여 있는 개발용 카탈로그다.
이미 있는 걸 또 만들지 않기 위한 것이니, 새 공통 컴포넌트를 만들기 전에 여기부터 확인한다
(프로덕션 빌드에는 포함되지 않는다).
...
- 프론트는 **공통 컴포넌트를 만들기 전에 `/dev/ui` 카탈로그를 먼저 확인**한다. 색상은 `tokens.css` 변수만 쓰고 HEX 하드코딩 금지
